...
::: {.proof}
*Proof.* By the discussion above, we have that for any key $\mathsf{k}$,
message $\mathsf{m}$ and ciphertext $\mathsf{c}$,
$$\Pr[\mathsf{c}=\mathsf{Enc}_{\mathsf{k}}(\mathsf{m})]=\frac{\#\{\text{keys $\mathsf{k}$ such that } \mathsf{Enc}_{\mathsf{k}}(\mathsf{m})\}}{\#\{\text{possible keys}\}}=\frac{1}{2^\lambda}.$$
:::
...
-   they cannot encrypt the key, since the don't have a shared key to
    use encryption yet!
...
1.  *$G$ outputs a string or $2\ell$ zeroes.*
...
to this date, is believed to be secure [@de2008trivium].
