...
::: {.definition}
*A* ring[^39] *is a triple $(\mathbb{K},\circ,\ast)$,
...
$\gcd(n,x)=1$, and compute $$x^{n-1}\bmod{p}.$$ If the result is not
...
test, which is why they are not often used in practice.[^40]
...
[^39]: In an abstract algebra context, the definition of a ring is more
...
[^40]: Agrawal, M., Kayal, N., & Saxena, N. (2004). PRIMES is in P.
