...
a generic group operation.[^20] In these notes, we will often use
...
```{proposition, name="Euler's theorem"}
...
above is known as *Fermat's little theorem*.[^21]
...
```{proposition, name="Chinese remainder theorem"}
*Let $a,b\in\mathbb{N}$
...
In this course we will be interested in *finite fields*,[^23] so what
...
$$a_{k-1}x^{k-1}+\dots +a_1x+a_0,$$ for some coefficients
$a_{k-1},\dots,a_0\in\mathbb{F}_p$. Note that the degree of the
...
use usual polynomial addition, that is, $$\begin{split}
    A(X)+B(X)& =(a_{k-1}X^{k-1}+\dots +a_1X+a_0)+(b_{k-1}X^{k-1}+\dots +b_1X+b_0) = \\
...
However, note that, in general, multiplying two polynomials $A(X)$ and
$B(X)$ of degree $k-1$ produces a polynomial $A(X)B(X)$ of degree
...
we compute the polynomial division of $A(X)B(X)$ by $T(X)$, and keep the
remainder, so that the result has degree at most $k-1$.[^24] Since this
...
[^20]: A prime example of a different group operation, which appears
    very often in cryptography, is the group law of elliptic curves.

[^21]: Not to be confused with *Fermat's last theorem*, which states
...
[^22]: Appendix \@ref(ring-theory) contains a more detailed introduction
...
[^23]: Sometimes also called Galois fields, after the mathematician
...
[^24]: See Appendix \@ref(polydiv) for a refresher on how to perform
    polynomial division.
